...
>
> The same bar applies to **plugins**, one level up — see [Plugins](#plugins) for the one that currently clears it.
...
The bar from the note above still applies: prefer native capabilities, then a cherry-picked skill in `ai/claude/skills/`, and only reach for a plugin when it delivers something neither can. One currently clears it.
