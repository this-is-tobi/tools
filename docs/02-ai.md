...
Claude Code reads skills from `.claude/skills/`. The `ai/common/skills/*/SKILL.md` files are already in Claude's native format (no conversion needed):
...
| Security Auditor     | Security-focused vulnerability analysis           |
| Refactorer           | Improves structure while preserving behavior      |
| Test Writer          | Generates tests following project conventions     |
| Doc Writer           | Generates and updates documentation from code     |
| Migration Assistant  | Dependency upgrades and framework migrations       |
| IaC Reviewer         | Reviews Terraform, K8s, Docker, CI/CD configs      |
